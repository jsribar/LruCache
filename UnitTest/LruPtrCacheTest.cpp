...
#include "..\TimedCache\LruPtrCache.h"
...
namespace UnitTest
{
	...
	public:
		...
		{
			{
				...
				int a = 5;
				LruPtrCache<MyStruct, int, decltype(my_generator)> cache{ my_generator, 0, 60000, 20000 };
				cache.GetItem(1);
				cache.GetItem(2);
				cache.GetItem(1);
		...
		TEST_METHOD(LruPtrCache_ContainsItemMethodReturnsFalseForEmptyCache)
		{
			...
			LruPtrCache<int, int, decltype(my_generator)> cache{ my_generator, 0, 60000, 20000 };
		...
		TEST_METHOD(LruCache_CleanupMethodRemovesItemIfItHasExpired)
		{
			auto my_generator = [](const int& a) -> int* { return new int(a * a); };
			size_t timeout = 10;
			LruPtrCache<int, int, decltype(my_generator)> cache{ my_generator, 0, timeout, 20000 };
			cache.GetItem(1);
		...
		TEST_METHOD(LruPtrCache_DefiningTDurationTemplateParameterInSecondsChangesTimeoutUnits)
		{
			auto my_generator = [](const int& a) -> int* { return new int(a * a); };
			size_t timeout = 2;
			LruPtrCache<int, int, decltype(my_generator), std::chrono::seconds> cache{ my_generator, 0, timeout, 20000 };
			cache.GetItem(1);
			std::this_thread::sleep_for(std::chrono::milliseconds(timeout + 1));
		...
		{
			LruPtrCache<int, int, int*(int)> cache{ NewSquare, 0, 0, 0 };
			cache.GetItem(3);
			Assert::IsTrue(cache.ContainsItem(3));
		}
	};
